...
public class PanneauGame extends JPanel implements MouseListener {
	...
	this.addMouseMotionListener(new MouseMotionListener() {
		...
		realPointeurX = (((PlayerList.get(0).getPosX() - width) + ((e
			.getX()) * 2)));
		realPointeurY = (((PlayerList.get(0).getPosY() - height) + ((e
			.getY()) * 2)));
		pointeurX = (int) ((PlayerList.get(0).getPosX() - width / 2
			/ zoom) / cellSizeX)
			+ (int) (((e.getX()) / zoom) / cellSizeX);
		pointeurY = (int) ((PlayerList.get(0).getPosY() - height / 2
			/ zoom) / cellSizeY)
	...
	g2.translate((-PlayerList.get(0).getPosX() * zoom) + this.getWidth()
		/ 2, (-PlayerList.get(0).getPosY() * zoom) + this.getHeight()
		/ 2);
	g2.scale(zoom, zoom);
	...
	if ((PlayerList.get(0).getPosX() - this.getWidth() / 2 / zoom)
		/ cellSizeX < 0) {
	    xMin = 0;
	} else {
	    xMin = (int) ((PlayerList.get(0).getPosX() - this.getWidth() / 2
		    / zoom) / cellSizeX);
	}

	if (((PlayerList.get(0).getPosX() + this.getWidth() / 2 / zoom) / cellSizeX) + 5 > lvl
		.getArraySizeX()) {
	    xMax = lvl.getArraySizeX();
	} else {
	    xMax = (int) ((PlayerList.get(0).getPosX() + this.getWidth() / 2
		    / zoom) / cellSizeX) + 5;
	}

	if ((PlayerList.get(0).getPosY() - this.getHeight() / 2 / zoom)
		/ cellSizeY < 0) {
	    yMin = 0;
	} else {
	    yMin = (int) ((PlayerList.get(0).getPosY() - this.getHeight() / 2
		    / zoom) / cellSizeY);
	}

	if (((PlayerList.get(0).getPosY() + this.getHeight() / 2 / zoom) / cellSizeY) + 5 > lvl
		.getArraySizeY()) {
	    yMax = (lvl.getArraySizeY());
	} else {
	    yMax = (int) ((PlayerList.get(0).getPosY() + this.getHeight() / 2
		    / zoom) / cellSizeY) + 5;
	}
	...
	for (Mob mob : MobList) {
		...
		for (int i = 0; i < mob.shortestPath.size(); i++) {
		    g2.fillRect(
			    (int) (mob.getPath().get(i).getX() * cellSizeX),
			    (int) (mob.getPath().get(i).getY() * cellSizeY),
			    (int) cellSizeX, (int) cellSizeY);
		}
	    }
	    // Fin dessin path
	    g2.drawImage(
		    rotate(mob.getImage(), (int) cellSizeX, (int) cellSizeY,
			    mob.getRotationWithVectors()),
		    (int) (mob.getPosX() - cellSizeX / 2),
		    (int) (mob.getPosY() - cellSizeY / 2), (int) cellSizeX,
		    (int) cellSizeY, this);
	...
	for (Player player : PlayerList) {
	    g2.drawImage(
		    rotate(player.getImage(), (int) cellSizeX, (int) cellSizeY,
			    player.getRotationWithMouse(realPointeurX,
				    realPointeurY)),
		    (int) (player.getPosX() - cellSizeX / 2), (int) (player
			    .getPosY() - cellSizeY / 2), (int) cellSizeX,
		    (int) cellSizeY, this);
